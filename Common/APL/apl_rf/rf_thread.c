...
uint8_t rf_Link_On = 0;                 /* ESP8266连接状态：0--未连接，1--智云连接成功，2--物理连接成功 */

static void rf_link_timer_cb(void* parameter) {
  static uint8_t led_flag = 0;
  switch(rf_Link_On) {
    case 0: led_off(LED_NET);   break;
    case 1: led_on(LED_NET);    break;
    case 2:
      led_flag = 1 - led_flag;
...
static void rf_thread_entry(void *parameter) {
  ...
  rf_if_init();
  rt_timer_t rf_link_timer = rt_timer_create("rf_link_timer", rf_link_timer_cb, RT_NULL, 500, RT_TIMER_FLAG_PERIODIC);
  rt_timer_start(rf_link_timer);
